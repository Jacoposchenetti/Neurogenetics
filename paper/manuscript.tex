...
(\emph{p}\_spin = 2 × 10⁻⁴). Three further tests dissolve that
conclusion. First, it is not band-specific: theta, beta and delta enrich
comparably or more strongly (top-100 beta \emph{p}\_spin = 0.011; delta
0.042; continuous theta 0.042). Second, it does not replicate across
alpha phenotypes (occipital alpha continuous \emph{p}\_spin = 0.20;
alpha peak frequency non-significant, \emph{p}\_spin ≥ 0.066). Third,
against random gene sets of matched size the alpha set is unremarkable
(\emph{p}\_geneset = 0.33) --- the apparent enrichment is a generic
property of arbitrary gene sets in this cortical territory, and is
invisible to a spatial-only null. No test survived false-discovery-rate
correction across the 24-cell phenotype × score × region-set grid
(minimum \emph{q} = 0.127), and nominal significance did not survive a
change of parcellation.

...
This reframes the §3.3 result entirely. The spin test asks whether the
\emph{brain map} could have arisen by chance given its spatial
smoothness, and answers no. It cannot ask whether the \emph{gene set} is
special, because it never permutes genes. The apparent enrichment is
therefore not evidence about alpha genetics at all --- it is a property
of how any co-expressed set of genes distributes over this cortical
territory, exactly the inflation Fulcher et al.~(2021) predict and that
a spatial-only null is structurally blind to.
...
after an unwelcome result, the reversal in §3.5 is a finding rather than
a post-hoc rescue.
...
band-power phenotypes conflate (§6).
